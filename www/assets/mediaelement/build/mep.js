...
(function (window, $, undefined) {
            ...
            if (settings.mepPlaylistTracks.length) {
                ...
                if (!$media.length) {
                    for (i = 0; i < trackCount; i++) {
                        ...
                        //logMessage(settings.mepPlaylistTracks[ i ].src);
    ...
    $.fn.mepPlaylist.defaults = {
        ...
        success: function (media, domObject, player) {
            ...
            // will take this to check if current track is been played on the new page loaded
    ...
    $.extend(MediaElementPlayer.prototype, {
        buildmepartwork: function (player, controls, layers) {
            ...
            player.$node.on('setTrack.mep', function (e, track, player) {
                ...
                $("#playlist").css(
                    'background-image', 'url(' + track.thumb.src + ')'
                );
    ...
    $.extend(MediaElementPlayer.prototype, {
        buildmepcurrentdetails: function (player, controls, layers) {
            ...
            player.$node.on('setTrack.mep', function (e, track, player) {
                ...
                incrementPlayed(trackId);
                incrementAlbumPlayed(trackId, function (data) {
                    incre(data.d);
                    increArtist(data.a, trackId);
                });
            });
        }
    });
...
function incrementPlayed(trackId) {
    FirebaseService.incrementValue({
        path: 'uploads/' + trackId + '/played',
        incrementBy: 1
    }, function (data) {
        //console.log(data);
    });
...
function incre(albumId) {
    FirebaseService.incrementValue({
        path: '/albums/' + albumId + '/played',
        incrementBy: 1
    }, function (data) {
        //console.log(data);
    });
...
function increArtist(artistId, trackId) {
    FirebaseService.incrementValue({
        path: '/uploads/'+trackId+'/'+artistId+'_played',
        incrementBy: 1
    }, function (data) {
        //console.log(data);
    });
...
function incrementAlbumPlayed(trackId, callback) {
    firebaseBaseDatabase.ref('uploads/' + trackId).once('value', function (snapshot) {
        var data = snapshot.val();
        var album = data.track_album;
        var artist = data.track_artist;
        callback({d:album,a:artist});
    });
}
    ...
    current = {
        ...
        updateTimeCodes: function () {
            ...
            if (!isNaN(duration)) {
                ...
                // 30 sec things here
                //logMessage(player.media.currentTime / duration * 100 );
            }
...
(function (window, $, undefined) {
    ...
    $.extend(MediaElementPlayer.prototype, {
        ...
        updatemepList: function () {
            ...
            logMessage($tracklist);
            player.$mepTracks = $playlist.find(player.options.mepSelectors.track);
        },
        createItem: function (item) {
            //logMessage(item);
            var tpl = '<li class="track">' +
                '<div class="track-action"><a class="track-remove">&times;</a></div>' +
                '<div class="playlist_image_holder"><span id="playlist_image" style="display: block; background-image: url(' + item.thumb.src + ')">' +
                '<div class="eq_holder hide"><span class="equalizer equalizer-inverse equalizer-16"><span class="equalizer-bar equalizer-bar-1"></span><span class="equalizer-bar equalizer-bar-2"></span><span class="equalizer-bar equalizer-bar-3"></span></span></div>' +
                '</span></div>' +
                '<div class="track-info">' +
                '<div class="track-title text-ellipsis" style="font-weight: 600;">' + item.title + '</div>' +
                '<div class="track-author text-ellipsis text-muted" style="margin-left: 0px;">by ' + item.meta.author + ' - ' + item.meta.duration + '</div>' +
                '</div>' +
                '</li>';
            return tpl;
        },
        ...
        addToQueque: function (item) {
            var player = this,
                $playlist = player.container.closest(player.options.mepSelectors.playlist),
                $tracklist = $playlist.find(player.options.mepSelectors.track),
                $tracks = $playlist.find(player.options.mepSelectors.track);

            var pos = player.mepCurrentTrack;
            var addToNextTrack = player.createItem(item);

            //logMessage(addToNextTrack);
            //logMessage($tracklist);
            //logMessage($tracks);

            $tracklist.splice(pos + 1, 0, addToNextTrack);
            $.each($tracklist, function (i) {
                $tracklist.append($tracklist[i]);
            });

            logMessage($tracklist);

            //$tracklist.append(addToNextTrack);
            // player.$mepTracks = $playlist.find(player.options.mepSelectors.track);
            ...
            // var n = 0;
            // for (n; n < $tracklist.length; n++) {
            //     var itemTrack = $tracklist[n];
            //     // if (itemTrack.indexOf("is-current") >= 0) {
            //     //     logMessage('got here');
            //     // }
            //     //var itemTra = $tracklist[n].find('.is-current');
            //     logMessage(itemTrack);
            // }

            // logMessage(item);
            // logMessage(addToQuequeTrack);
            // logMessage($tracks);
            // logMessage($tracklist);
            // logMessage($see);

            //
            // var addToQuequeTrack = $tracklist.splice(player.mepCurrentTrack + 1, 0, addToQuequeTrack);
            // logMessage($tracklist);
            // return;
            ...
            // // logMessage(addToQuequeTrack);
            // // logMessage($tracklist);
            // logMessage(player.mepCurrentTrack);
            // player.$mepTracks = $playlist.find(player.options.mepSelectors.track);
